...
function Recent(){
       ...
       const Filter =(data)=>{
                setRecords(data.filter(f=>f.position.includes("completed")))
       }
    ...
    {records.map((data,index)=>(
        ...
        <tbody   key={index}>
          <tr>
            ...
            <td>{data.position}</td>
            <td type="button" 
            className='button' onClick={()=>deleteRecent(data._id)}><FontAwesomeIcon className="deleteIcon"icon={faTrash} /></td>
          </tr>
        </tbody>
    ))}
    </table>
